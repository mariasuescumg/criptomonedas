import {useEffect, useState} from 'react' // se Hooks para manipular los estados  de los componentes 
// se importan las los componenete y los estilos utilizados en este componente principal
...
// se declara la fucion App
export default function App() {
  const [coins, setCoins] = useState()
  const [currency, setCurrency] = useState()
  const [selCur, setSelCur] = useState("usd")
  const getData = async () =>{
    const response = await fetch(`https://api.coingecko.com/api/v3/coins/markets?vs_currency=${selCur}&order=market_cap_desc&per_page=3&page=1&sparkline=false&price_change_percentage=1h%2C24h%2C7d%2C30d%2C90d%2C1y`)
